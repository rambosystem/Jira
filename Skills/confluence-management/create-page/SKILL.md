...
- **若** 返回 `results` 非空：取 `results[0].id` 为 **page_id**，进入步骤 4（更新）。
...
```json
{
  ...
  "type": "doc",
  "content": [
    { "type": "paragraph", "content": [{ "type": "text", "text": "段落文字" }] },
    { "type": "blockCard", "attrs": { "url": "https://site.atlassian.net/browse/PROJ-123" } }
  ]
}
```
...
- **本仓库脚本示例**（创建页面 / ADF）：`scripts/confluence_create_page.py`。从 profile 读 confluence_*；认证用环境变量 `CONFLUENCE_EMAIL`、`CONFLUENCE_API_TOKEN`。示例：`python scripts/confluence_create_page.py --title "标题" [--jira-url "https://.../browse/PIN-123"]`。
