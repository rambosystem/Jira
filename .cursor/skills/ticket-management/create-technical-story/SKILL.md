...
3. **先 dry-run**：运行 `python3 scripts/jira/create_story.py --issue-type "Technical Story" ... --dry-run`，查看 preflight。
4. **再创建**：去掉 `--dry-run`；如需允许重复，追加 `--allow-duplicate`。
5. **PIN 关联**：若用户给 PIN key，命令追加 `--link-pin PIN-1,PIN-2`（逗号分隔），脚本自动逐个建立 Relates。
...
- 优先用 `scripts/jira/create_story.py --issue-type "Technical Story"`；查询统一用 `scripts/jira/query_issues.py --jql "<JQL>"`，不要手动拼 MCP 查询/创建参数；仅脚本不可用时退回 MCP-tools.md。
