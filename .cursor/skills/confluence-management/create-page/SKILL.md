...
- **脚本**：`scripts/confluence_create_page.py` 已实现「先按标题查找 → 存在则 GET 并合并 ADF 后 PUT，否则 POST」，支持 `--body-file`、`--body-json`、`--jira-url`。
...
- **本仓库脚本**：`scripts/confluence_create_page.py`。从 profile 读 confluence_*；认证用 `CONFLUENCE_EMAIL`、`CONFLUENCE_API_TOKEN`。支持 `--title`、`--body-file`、`--body-json`、`--jira-url`；按标题先查页，存在则追加、否则新建。示例：`python scripts/confluence_create_page.py --title "2026-03-11 Processed" --body-file report.json`。
