...
在 **Confluence** 的指定空间、指定父页面（文件夹）下，按**页面标题**创建新页面或更新已有页面。目标空间与父文件夹来自 **`config/assets/global/profile.yaml`** 的 **`confluence_workspace`**。本技能使用 **Confluence Cloud REST API v2** 直接调用，不依赖 MCP。
...
- **`config/assets/global/profile.yaml`**：从以下**固化字段**读取 Confluence 目标，无需解析 URL 或调用 GET /spaces：
...
- **密码**：Atlassian API Token（从环境变量 `ATLASSIAN_API_TOKEN` 或调用方提供的凭证获取；**勿在代码中硬编码**）。
...
- 从 **`config/assets/global/profile.yaml`** 读取：**confluence_base_url**、**confluence_space_id**（数字）、**confluence_parent_id**（可选保留 **confluence_space_key** 用于展示）。无需解析 URL，无需调用 GET /spaces。
...
- **执行方式**：页面创建/更新**一律**通过 **`scripts/confluence/confluence_create_page.py`** 执行。调用方（如 request-pin-report）**只生成 ADF Body JSON**（不为此编写 Python 或其他脚本），通过 `--body-file`（或 `--body-json`、`--jira-url`）传入；脚本已实现「先按标题查找 → 存在则 GET 并合并 ADF 后 PUT，否则 POST」。
...
- **本仓库脚本**：`scripts/confluence/confluence_create_page.py`。从 profile 读 confluence\_\*；认证用 `CONFLUENCE_EMAIL`、`ATLASSIAN_API_TOKEN`。支持 `--title`、`--body-file`、`--body-json`、`--jira-url`；按标题先查页，存在则追加、否则新建。

### 执行脚本（终端）

- **工作目录**：须在仓库根目录执行（脚本会读 `config/assets/global/profile.yaml` 与相对路径的 `--body-file`）。
- **Windows PowerShell**：不要使用 `cd /d ... && python ...`，因 **`&&` 在 PowerShell 5.x 中不是有效语句分隔符**。请用以下任一方式：
  - 分号连接：`Set-Location <仓库根路径>; python scripts/confluence/confluence_create_page.py --title "2026-03-11 Processed" --body-file report.json`
  - 或先切换目录再执行：先 `Set-Location <仓库根路径>`，再单独运行 `python scripts/confluence/confluence_create_page.py ...`。
- **示例（PowerShell）**：`Set-Location e:\Workspace\Jira; python scripts/confluence/confluence_create_page.py --title "2026-03-11 Processed" --body-file report.json`

## Guardrails

- **一律使用脚本**：Confluence 页面的创建/更新**必须**通过 **`scripts/confluence/confluence_create_page.py`** 执行；调用方只提供 ADF 正文（如写入临时 JSON 后 `--body-file`），**禁止**为发布 Confluence 页面而编写新的 Python 或其他脚本。
