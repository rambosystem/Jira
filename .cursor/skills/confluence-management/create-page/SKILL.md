...
- **执行方式**：页面创建/更新**一律**通过 **`Scriptsconfluence/confluence_create_page.py`** 执行。调用方（如 request-pin-report）**只生成 ADF Body JSON**（不为此编写 Python 或其他脚本），通过 `--body-file`（或 `--body-json`、`--jira-url`）传入；脚本已实现「先按标题查找 → 存在则 GET 并合并 ADF 后 PUT，否则 POST」。
...
- **本仓库脚本**：`Scriptsconfluence/confluence_create_page.py`。从 profile 读 confluence\_\*；认证用 `CONFLUENCE_EMAIL`、`ATLASSIAN_API_TOKEN`。支持 `--title`、`--body-file`、`--body-json`、`--jira-url`；按标题先查页，存在则追加、否则新建。
...
- **Windows PowerShell**：不要使用 `cd /d ... && python ...`，因 **`&&` 在 PowerShell 5.x 中不是有效语句分隔符**。请用以下任一方式：
  - 分号连接：`Set-Location <仓库根路径>; python Scriptsconfluence/confluence_create_page.py --title "2026-03-11 Processed" --body-file report.json`
  - 或先切换目录再执行：先 `Set-Location <仓库根路径>`，再单独运行 `python Scriptsconfluence/confluence_create_page.py ...`。
- **示例（PowerShell）**：`Set-Location e:\Workspace\Jira; python Scriptsconfluence/confluence_create_page.py --title "2026-03-11 Processed" --body-file report.json`

## Guardrails

- **一律使用脚本**：Confluence 页面的创建/更新**必须**通过 **`Scriptsconfluence/confluence_create_page.py`** 执行；调用方只提供 ADF 正文（如写入临时 JSON 后 `--body-file`），**禁止**为发布 Confluence 页面而编写新的 Python 或其他脚本。
