# PAG 项目创建工单字段（Create Metadata）

由 `scripts/jira/get_createmeta.py PAG` 从 Jira API 拉取。  
**与 CP 项目结构一致**（相同 issue 类型与必填/可选字段）。
...
**可选**: Assignee, Epic Link, Sprint, Tier (customfield_10047), Fixer (customfield_10053), Description, Due date, Fix versions, Labels, Parent, Time tracking, Affects versions
...
更新：`python3 scripts/jira/get_createmeta.py PAG`
